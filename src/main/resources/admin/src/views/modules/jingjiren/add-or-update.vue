  ...
  data() {
	...
	shoujihao : false,
      ...
      rules: {
          ...
          jingjirenming: [
                { required: true, message: '经纪人名不能为空', trigger: 'blur' },
          ],
          mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jingjiren/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jingjirenCrossAddOrUpdateFlag = false;
						 this.parent.search();
